...
class EGraphHeuristic{
  ...
  public:
    ...
    // For MAS, set the agent id
    virtual void setAgentId(int agentid){};
    
    // If the heuristic is computed by down-projection, set the heuristic grid
    virtual void setGrid(const std::vector<std::vector<bool> >& grid){};
   ...
   // add a clone method for Multiagent planning
